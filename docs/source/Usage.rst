...
| Alternatively, you can extract a DEM from OpenTopography. To do so, you will need first to request an API key from OpenTopography website (`request api key <https://opentopography.org/blog/introducing-api-keys-access-opentopography-global-datasets>`_ ). Then copy your key in a text file, name it "OpenTopoKey.txt", and put the file in the root directory of PecubeGUI application (i.e., the same location than the Pecube directory). To download a DEM of your region of interest (ROI), provide the coordinates of the bottom left corner of the topography and its resolution in the "Topography" tab (i.e., nx, ny, longitude 0, latitude 0, Δlon, Δlat). Click on "Get topo..." button will then extract the DEM. This option uses the python package bmi_topography [Piper-2021]_ to download the DEM from the SRTMGL3 (90 m resolution) hosted by OpenTopography. According to the size of your ROI it can take few second to download. Once the DEM is downloaded, it can be found in "your_current_project/data/" directory. Click on "load file(s)" (Figure 2) to load the DEM within the interface. 
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 Enabling \ :sup:`4`\He/\ :sup:`3`\He prediction opens a window from which to provide observed spectra (Figure 12). The window starts with an information text explaining how to provide each spectrum. The steps are: 1. Select the grain from wich to provide an observed \ :sup:`4`\He/\ :sup:`3`\He profile, 2. Set the number of heating steps, 3. Provide the heating schedule (optional), time is in hours and temperature in Celsius degree. 4. Provide the cumulative sum of fractional 3He (F3He) with the associated uncertainty (Error F). 5. Provide the 4He/3He ratio (i.e., Rs/Rb: Rstep/Rbulk) with its associated uncertainty (Error R). 6. Click on ‘add grain’ to add the \ :sup:`4`\He/\ :sup:`3`\He profile for the selected grain. The grain’s ID is shown on the list in 7. To remove a \ :sup:`4`\He/\ :sup:`3`\He profile for a grain, select the targeted grain on the list in 7 and click on “Remove grain” (8). Once, all 4He/3He profiles for targeted grain are provided click on the Ok button to save the profiles. The input file with \ :sup:`4`\He/\ :sup:`3`\He data is then saved in your Pecube project folder “Project_name/data/data_folder_name/He43_data.csv”.
...
* *Sample size for all other iteration*: number of Pecube forward models to run in all other iterations.
