...
.. note::
  ...
  Where p = 2 (assuming Gaussian distribution of errors), S^{obs}_j the observed data j and S^{pred}_j the predicted data j, \sigma_j the error on the observed data j,
  and N the total number of observed data. The higher the value of LL, the better is the match between observed and predicted data. 
