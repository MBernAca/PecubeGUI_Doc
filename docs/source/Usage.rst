...
| Alternatively, you can extract a DEM from an interactive map included in the interface. Click on "Get topo..." button will show the map in which you can draw a rectangle area of your region of interest (Figure 4). After drawing the area of interest, click on it to extract the coordinates, then close the window. This option use the python package bmi_topography (Piper, 2021) to download the DEM from the SRTMGL1 (90 m resolution) hosted by OpenTopography. To be able to download the DEM you have to request a API-key from OpenTopography (`request api key <https://opentopography.org/blog/introducing-api-keys-access-opentopography-global-datasets>`_ ). The DEM can now be found in "your_current_project/data" directory with the name "topoRaster.tif". Click on "load file(s)" (Figure 2) to load the DEM within the interface. 
